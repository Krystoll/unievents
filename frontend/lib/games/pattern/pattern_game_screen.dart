...
class _PatternGameScreenState extends State<PatternGameScreen> {
  ...
  void _startDrag(int index, Offset globalPosition) {
    if (_gameOver || _isAnimatingClear) return;
    setState(() {
  ...
  Future<void> _tryPlaceBlock(int row, int col) async {
    if (_draggingBlockIndex == null || _gameOver || _isAnimatingClear) return;
    ...
    if (lines.count > 0) {
      ...
      setState(() {
        ...
        _score += calculateScore(lines.count);
        _isAnimatingClear = false;
        _clearingLines = const LinesToClear(rows: [], cols: []);
      });
    } else {
            ...
            header: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text('Очки: $_score', style: Theme.of(context).textTheme.titleSmall),
                    Text('Линий: $_linesCleared', style: Theme.of(context).textTheme.titleSmall),
                  ],
                ),
                Text(
                  _draggingBlockIndex != null
                      ? 'Перетащите фигуру на поле'
                      : 'Зажмите и перетащите фигуру',
